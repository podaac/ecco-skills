...
#
# ---------------------------------------------------------------------------
# VENDORED into ecco-skills 2026-08-05 from the ECCO v4 Python Tutorial repo
#   ECCO-GROUP/ECCO-v4-Python-Tutorial @ commit 3f0fcca (path: misc/jmd95.py)
#   (same pinned commit as the vendored ecco_po_tutorials.py).
# It is the canonical MITgcm Jackett & McDougall (1995) equation of state.
#
# ONLY MODIFICATION: `np.asfarray(x)` → `np.asarray(x, dtype=float)` (6 sites) —
# np.asfarray was removed in NumPy 2.0 and our env pins numpy>=2.5. The two are
# exactly equivalent (asfarray == asarray with a float dtype). No numerical change:
# the published check value densjmd95(35.5, 3, 3000) = 1041.83267 still reproduces.
# Do not otherwise edit this file — it is a pinned upstream reference (Rung-1 anchor).
# ---------------------------------------------------------------------------
